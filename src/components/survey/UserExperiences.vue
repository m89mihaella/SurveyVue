<template>
  <section>
    <base-card>
      ...
      <p v-else-if="!isLoading && (!results || results.length === 0)">No experiences found. Start by adding some experiences.</p>
  ...
  methods: {
    loadExperiences() {
      ...
      fetch("https://survey-vue-564c4-default-rtdb.europe-west1.firebasedatabase.app/surveys.json")
        ...
        .catch((error) => {
          this.isLoading = false;
          this.error = error.message || "Could not fetch data. Try again later.";
        });
    },
  },
  mounted() {
